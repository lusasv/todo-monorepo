...
0. **CRITICAL — Branch**: You are running inside a git worktree. Branch `us-analise-essa-tela-https-www-figma-com-de` is **already checked out**. Do NOT run `git checkout` or `git checkout -b` — it will fail. ALL work MUST happen on this branch. NEVER commit to `feature/todo-api` or `main`.
...
- Task: analise essa tela:https://www.figma.com/design/ik0Qa30O9oNUy3qelJbQO7/Ferramenta-Trello?t=UUR6mONLZBORBbdk-0
...
Platform documentation is stored at `/Users/user/tmp/_squad_remote/run-390/repo/docs/`. This is OUTSIDE the repository — it will not conflict with the repo's own files.
Instruct agents to use absolute paths when reading or writing these docs.

**Available docs:**
- `/Users/user/tmp/_squad_remote/run-390/repo/docs/ARCHITECTURE.md` — system architecture, components, data flow
- `/Users/user/tmp/_squad_remote/run-390/repo/docs/CONVENTIONS.md` — coding patterns and naming standards
- `/Users/user/tmp/_squad_remote/run-390/repo/docs/API.md` — API endpoints documentation
- `/Users/user/tmp/_squad_remote/run-390/repo/docs/DECISIONS.md` — architecture decision log (append-only)
- Check `ls /Users/user/tmp/_squad_remote/run-390/repo/docs/` for additional docs and subdirectories

**Rules for agents:**
- **Before coding:** instruct Backend/Frontend agents to read `/Users/user/tmp/_squad_remote/run-390/repo/docs/CONVENTIONS.md`
- **After architectural decisions:** instruct agents to append to `/Users/user/tmp/_squad_remote/run-390/repo/docs/DECISIONS.md` with date, decision, and rationale
- **After new APIs:** instruct Backend agent to update `/Users/user/tmp/_squad_remote/run-390/repo/docs/API.md`
